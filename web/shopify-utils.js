...
const getActiveThemeId = async (session) => {
  try {
    const response = await axios.get(`https://${shopifyDomain}/admin/api/2023-10/themes.json`, {
      headers: {
...
const updateThemeTemplate = async (themeId, session) => {
  try {
    await axios.put(`https://${shopifyDomain}/admin/api/2023-10/themes/${themeId}/assets.json`, {
      asset: {
